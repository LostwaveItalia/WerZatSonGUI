...
![Versione: 2.1.0](https://img.shields.io/badge/Versione-2.1.0-orange)
...
- Una finestra **Seleziona cartelle PKLZ...**: un albero del tuo database Audfprint, con ricerca e ordinamento, che ha una casella su ogni cartella **e su ogni singolo file `.pklz`**, permettendoti di restringere una scansione esattamente ai fingerprint che vuoi invece di cercare sempre nell'intero database. Ogni riga mostra quanti file PKLZ contiene e quanto spazio occupano, e qualsiasi cosa selezioni viene cercata in un'unica passata.
...
> **ATTENZIONE: Un account AudioTag gratuito è limitato a circa 1.000 richieste di identificazione al mese.** Contro tracce sconosciute reali, quel budget può esaurirsi anche dopo soli 100-200 file, quindi AudioTag da solo non è adatto a scansionare migliaia di tracce. WerZatSonGUI aggiunge un tempo di attesa casuale tra le richieste, una pausa di sicurezza periodica, e una rotazione facoltativa tra le chiavi di più account gratuiti per usare questo budget in modo più attento; vedi la **scheda AudioTag** nelle Impostazioni avanzate più sotto. Nulla di tutto ciò elimina i limiti stessi di AudioTag, quindi in ogni caso resta attento al loro servizio.
...
Suddivise in nove schede in modo che le impostazioni correlate siano raggruppate insieme. Ogni singola impostazione ha un piccolo pulsante **[?]** alla sua sinistra con una breve spiegazione, e questa sezione ne riassume anche il funzionamento. L'intero pannello appare in grigio mentre è in corso una scansione.
...
- **Riepilogo della selezione:** un breve riepilogo di ciò che è attualmente selezionato appare proprio accanto ai due pulsanti qui sopra, combinando entrambe le selezioni. Appare più o meno così: `10 canzoni, 2 cartelle di fingerprint, 2 file di fingerprint [9 MP3, 1 M4A, 16 PKLZ, 500.00 MB].`. Quando si usa l'intero database di fingerprint, vengono mostrate solo le canzoni (`10 canzoni, intero database di fingerprint.`), e viceversa (`Tutte le canzoni, 2 cartelle di fingerprint, 2 file di fingerprint [...]`). Passandoci sopra col mouse, vengono elencati i nomi effettivi delle canzoni e delle cartelle/file sotto le intestazioni in grassetto **Canzoni:** e **Fingerprint:**, con ogni canzone annotata con la/e modalità per cui è selezionata (**Veloce**, **Lunga** o **Entrambe**). Non viene mostrato nulla finché almeno una delle due finestre non è stata salvata almeno una volta; prima di allora viene mostrato un testo segnaposto.
...
Vedi l'avviso sotto "Come ottenere una chiave API di AudioTag" più sopra per il motivo di queste impostazioni: il budget di circa 1.000 richieste/mese di un account AudioTag gratuito si esaurisce in fretta contro tracce sconosciute reali, quindi questi controlli servono a scaglionare le richieste e, facoltativamente, a distribuirle su più account.

- **Tempo di attesa tra le richieste (secondi), min:max:** prima di ogni richiesta ad AudioTag, WerZatSonGUI attende un numero casuale di secondi in questo intervallo (predefinito 10-30), così le richieste non vengono inviate una dopo l'altra.
- **Pausa (e rotazione delle chiavi) dopo questo numero di tracce:** un checkpoint di sicurezza periodico (predefinito 100 tracce). Nota che questo conta le tracce, non le richieste grezze: la ricerca di una singola traccia comporta già una richiesta di identificazione più diversi controlli di stato gratuiti. A questo checkpoint WerZatSonGUI effettua sempre la pausa indicata sotto e, se Usa più chiavi API di AudioTag è attivo ed è disponibile un'altra chiave utilizzabile, passa anche a quella.
- **Durata della pausa a quel checkpoint (secondi):** per quanto tempo dura quella pausa periodica (predefinito 300 = 5 minuti). Si applica indipendentemente dal fatto che siano configurate più chiavi.
- **Durata minima della clip inviata ad AudioTag (secondi):** il server di AudioTag rifiuta nella pratica le clip molto corte. Qualsiasi clip più corta di questo valore (predefinito 15) viene ripetuta in loop (ripetendo il proprio audio, non riempita con silenzio) fino a raggiungere questa lunghezza prima di essere inviata, così le clip corte hanno comunque una possibilità di essere identificate invece di essere saltate silenziosamente.
- **Usa più chiavi API di AudioTag:** fa ruotare le ricerche AudioTag tra un elenco di chiavi (ciascuna proveniente da un account gratuito separato) invece della singola chiave in Chiavi API e Webhook (.env). Attivandola compare un avviso: usare più chiavi significa comunque colpire il server di AudioTag più spesso nel complesso, quindi resta attento al loro servizio. Le chiavi si gestiscono con i pulsanti Aggiungi chiave.../Rimuovi selezionata sotto la casella, e sono salvate in `assets\audiotag_keys.json`; WerZatSonGUI contrassegna automaticamente una chiave come esaurita o non valida in base alle risposte del server, e passa alla successiva chiave utilizzabile; l'elenco mostra il numero di tracce usate e lo stato di ogni chiave (mostrando solo le ultime 4 cifre).
...
- **AudioTag:** invia il file all'API di [AudioTag.info](https://audiotag.info) e riporta qualsiasi corrispondenza trovi. Richiede una **chiave API di AudioTag**. Vedi la scheda AudioTag più sopra per le impostazioni di tempo di attesa/pausa/multi-chiave che scaglionano le richieste entro i limiti del piano gratuito di AudioTag, e l'avviso sotto "Come ottenere una chiave API di AudioTag". Ogni chiamata ad AudioTag effettuata da questa modalità (che porti o meno a una corrispondenza) viene registrata per intero in `_audiotag_activity.jsonl` e `_audiotag_debug.jsonl` dentro la cartella dei risultati di quella esecuzione, così gli errori sono facili da diagnosticare.
...
Selezionare più cartelle PKLZ significava prima una scansione completa per ciascuna cartella, con la modalità Lunga che rigenerava ogni variazione di velocità/tonalità da zero ogni volta. Questo non succede più: la selezione viene raccolta in un'unica cartella di staging prima della scansione (vedi [*Canzoni processate e finestre di selezione*](#canzoni-processate-e-finestre-di-selezione) più sotto), quindi una selezione di qualsiasi forma è una sola esecuzione e le variazioni vengono generate una volta sola.
...
Per cambiare quali fingerprint vengono cercati da una scansione, apri **Seleziona cartelle PKLZ...**. Mostra la cartella del tuo database Audfprint come un albero con una casella su ogni cartella e su ogni singolo file `.pklz`, più una colonna **File** e una **Dimensione**, così puoi vedere quanto pesa davvero una riga prima di selezionarla. Selezionare una cartella include tutto ciò che contiene; selezionare qualcosa più in profondità annulla la scelta più ampia sopra di essa. Una cartella il cui contenuto è selezionato solo in parte mostra un terzo stato, "parziale".

Quando **Usa l'intero database** è selezionato, ogni riga appare selezionata, così l'intera selezione è visibile a colpo d'occhio. Cliccare una riga per deselezionarla disattiva automaticamente **Usa l'intero database** e prepara immediatamente la selezione rimanente in `___TEMP`; nulla viene spostato finché la casella è ancora selezionata, perché viene cercato direttamente nella radice del database. Se in seguito riselezioni tutto (a mano, riga per riga), **Usa l'intero database** si riseleziona da solo e la scansione torna alla radice del database (nessun flag `--folder`, nessuna preparazione in `___TEMP`).

Le cartelle partono **chiuse** e il loro contenuto viene caricato solo quando le apri: è questo che rende la finestra utilizzabile su un database grande (un database da 30.000 file si apre in circa due secondi). Il campo **Cerca** filtra tutte le cartelle e i file insieme, mostrando i risultati come percorsi completi; cliccando l'intestazione di una colonna si ordina per quella colonna, e cliccandola di nuovo si inverte l'ordine. **Deseleziona tutto** svuota la selezione senza toccare **Usa l'intero database**.

Audfprint può essere puntato solo su una cartella, quindi salvando una selezione i file `.pklz` scelti vengono **spostati** fisicamente in una cartella `___TEMP` dentro il tuo database che ne rispecchia la struttura, e la scansione cerca lì. Deselezionandoli vengono riportati indietro, e **Usa l'intero database** riporta tutto ed elimina del tutto `___TEMP`, quindi ogni file si trova sempre o al suo posto originale o in `___TEMP`, mai altrove. Gli spostamenti avvengono quando premi **Salva**, dietro una finestra di avanzamento, e vengono riapplicati all'inizio di ogni scansione, così la cartella di staging corrisponde sempre a ciò che hai salvato anche se nel frattempo dei file sono stati aggiunti o spostati a mano. Niente viene mai sovrascritto: se un file esiste già nella destinazione lo spostamento viene saltato e viene stampato un `[ATTENZIONE]` che lo nomina.

Per cambiare quali canzoni sono in attesa, apri **Seleziona canzoni...** (vedi la **scheda Generali** sopra): mostra ogni canzone della tua Cartella di input come un albero ricercabile e ordinabile, con una casella **Veloce** e una **Lunga** su ogni riga, più una colonna **Tipo di file** (vuota per le cartelle) e le colonne **File** / **Dimensione** (aggregate attraverso le cartelle). Cliccare la casella di una cartella cambia tutte le canzoni al suo interno in una volta sola. Il campo **Cerca** filtra canzoni e cartelle contemporaneamente (corrispondendo al percorso relativo); cliccando l'intestazione di una colonna si ordina per quella colonna, e cliccandola di nuovo si inverte l'ordine. Questa finestra è dove ora vive il vecchio flusso di lavoro "Segna tutti i file audio come processati in", senza dover poi modificare un file di testo a mano.
...
- **WerZatSonGUI v2.1.0** di some random account, con contributi da EierkuchenHD, VoidGod, Mystic65 e Numerophobe. Tester: EierkuchenHD, VoidGod, Shardanik, AuDriūnas, Cluttic, Simon Le Plot, drpostal, gabry4072_, Mystic65. Traduzione in italiano a cura di some random account.
