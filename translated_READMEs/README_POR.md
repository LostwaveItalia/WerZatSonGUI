...
![Versão: 2.1.0](https://img.shields.io/badge/Versão-2.1.0-orange)
...
- Uma janela **Selecionar pastas PKLZ...**: uma árvore do seu banco de dados Audfprint, com busca e ordenação, com uma caixa de seleção em cada pasta **e em cada arquivo `.pklz` individual**, permitindo restringir uma varredura exatamente aos fingerprints desejados em vez de sempre buscar em todo o banco de dados. Cada linha mostra quantos arquivos PKLZ contém e quanto espaço ocupam, e tudo o que você selecionar é buscado em uma única passagem.
...
> **AVISO: Uma conta gratuita do AudioTag é limitada a cerca de 1.000 solicitações de identificação por mês.** Contra faixas desconhecidas reais, esse orçamento pode se esgotar em apenas 100-200 arquivos, então o AudioTag sozinho não é adequado para varrer milhares de faixas. O WerZatSonGUI adiciona um tempo de espera aleatório entre as solicitações, uma pausa de segurança periódica e uma rotação opcional entre as chaves de várias contas gratuitas para usar esse orçamento com mais cuidado; veja a [*aba AudioTag*](#aba-audiotag) nas Configurações Avançadas abaixo. Nada disso remove os limites do próprio AudioTag, então continue atento ao uso do serviço deles de qualquer forma.
...
Dividido em nove abas para agrupar configurações relacionadas. Cada configuração individual tem um pequeno botão **[?]** à esquerda com uma breve explicação, e esta seção também resume o que cada uma faz. Todo este painel fica acinzentado enquanto uma varredura está em execução.
...
- **Resumo da seleção:** um breve resumo do que está selecionado no momento aparece ao lado dos dois botões acima, combinando as duas seleções. Algo como `10 músicas, 2 pastas de impressões digitais, 2 arquivos de impressões digitais [9 MP3(s), 1 M4A(s), 16 PKLZ(s), 500.00 MB].`. Quando o banco de dados de impressões digitais completo é usado, apenas as músicas são reportadas (`10 músicas, banco de dados de impressões digitais completo.`), e vice-versa (`Todas as músicas, 2 pastas de impressões digitais, 2 arquivos de impressões digitais [...]`). Passar o mouse sobre ele lista os nomes reais das músicas e das pastas/arquivos sob cabeçalhos em negrito **Músicas:** e **Impressões digitais:**, com cada música anotada com o(s) modo(s) para o(s) qual(is) ela está marcada (**Rápido**, **Longo** ou **Ambos**). Nada é mostrado até que pelo menos uma das duas janelas tenha sido salva uma vez; antes disso, um texto de espaço reservado é exibido.
...
Veja o aviso em "Como Obter uma Chave de API do AudioTag" acima para entender por que essas configurações existem: o orçamento de ~1.000 solicitações/mês de uma conta gratuita do AudioTag desaparece rapidamente diante de faixas desconhecidas reais, então esses ajustes existem para controlar o ritmo das solicitações e, opcionalmente, distribuí-las entre várias contas.

- **Tempo de espera entre solicitações (segundos), mín:máx:** antes de cada solicitação ao AudioTag, o WerZatSonGUI aguarda um número aleatório de segundos dentro desse intervalo (padrão `10`-`30`), para que as solicitações não sejam enviadas uma atrás da outra.
- **Pausar (e rotacionar chaves) após esta quantidade de faixas:** um checkpoint de segurança periódico (padrão `100` faixas). Observe que isso conta faixas, não solicitações brutas: a busca de uma única faixa já envolve uma solicitação de identificação mais várias verificações de status gratuitas. Nesse checkpoint, o WerZatSonGUI sempre faz a pausa abaixo e, se **Usar várias chaves de API do AudioTag** estiver ativado e outra chave utilizável estiver disponível, também muda para ela.
- **Duração da pausa nesse checkpoint (segundos):** por quanto tempo essa pausa periódica dura (padrão `300` = 5 minutos). Aplica-se independentemente de várias chaves estarem configuradas ou não.
- **Duração mínima do clipe enviado ao AudioTag (segundos):** na prática, o servidor do AudioTag rejeita clipes muito curtos. Qualquer clipe mais curto do que este valor (padrão `15`) é repetido em loop (reproduzindo seu próprio áudio novamente, sem preenchimento com silêncio) até atingir essa duração antes de ser enviado, para que clipes curtos ainda tenham uma chance de identificação em vez de serem ignorados silenciosamente.
- **Usar várias chaves de API do AudioTag:** alterna as buscas do AudioTag entre uma lista de chaves (cada uma de uma conta gratuita separada) em vez da chave única em **Chaves de API e Webhook (.env)**. Ativar essa opção mostra um aviso: usar várias chaves ainda significa atingir o servidor do AudioTag mais vezes no total, então continue atento ao uso do serviço deles. As chaves são gerenciadas com os botões **Adicionar chave...**/**Remover selecionada(s)** abaixo da caixa de seleção e são armazenadas em `assets\audiotag_keys.json`; o WerZatSonGUI marca automaticamente uma chave como esgotada ou inválida com base nas respostas do próprio servidor e passa para a próxima chave utilizável, e a lista mostra a contagem de uso e o status de cada chave (mascarada até os últimos 4 caracteres).
...
- **AudioTag:** envia o arquivo para a API do [AudioTag.info](https://audiotag.info) e retorna qualquer correspondência que encontrar. Requer uma **chave de API do AudioTag**. Veja a [*aba AudioTag*](#aba-audiotag) acima para as configurações de tempo de espera/pausa/múltiplas chaves que controlam o ritmo das solicitações dentro dos limites do plano gratuito do AudioTag, e o aviso em "Como Obter uma Chave de API do AudioTag". Toda chamada ao AudioTag feita por esse modo (com ou sem correspondência) é totalmente registrada em `_audiotag_activity.jsonl` e `_audiotag_debug.jsonl` dentro da pasta de resultados dessa execução, para que falhas sejam fáceis de diagnosticar.
...
Selecionar várias pastas PKLZ antes significava uma varredura completa por pasta, com o Modo Longo regenerando cada variação de tempo/afinação do zero a cada vez. Isso não acontece mais: a seleção é reunida em uma única pasta de preparação antes da varredura (veja mais abaixo), então uma seleção de qualquer formato é uma única execução e as variações são geradas uma só vez.
...
Para mudar quais fingerprints uma varredura busca, abra **Selecionar pastas PKLZ...**. A janela mostra o diretório do seu banco de dados do Audfprint como uma árvore, com uma caixa de seleção em cada pasta e em cada arquivo `.pklz`, além de uma coluna **Arquivos** e uma **Tamanho**, para você ver quanto uma linha realmente custa antes de marcá-la. Marcar uma pasta cobre tudo o que está dentro dela; marcar algo mais profundo desfaz a escolha mais ampla acima. Uma pasta cujo conteúdo está selecionado apenas em parte mostra um terceiro estado, "parcial".

Quando **Usar banco de dados completo** está marcado, cada linha é exibida como marcada, de modo que toda a seleção é visível de relance. Clicar em qualquer linha para desmarcá-la desliga automaticamente **Usar banco de dados completo** e prepara imediatamente a seleção restante em `___TEMP`; nada é movido enquanto a caixa ainda estiver marcada, pois a raiz bruta do banco de dados é pesquisada diretamente. Se você marcar tudo novamente mais tarde (manualmente, linha por linha), **Usar banco de dados completo** se remarca sozinho e a varredura volta à raiz bruta do banco de dados (sem sinalizador `--folder`, sem preparação em `___TEMP`).

As pastas começam **fechadas** e seu conteúdo só é carregado quando você as abre, o que é o que mantém a janela utilizável em um banco de dados grande (um banco com 30.000 arquivos abre em cerca de dois segundos). O campo **Pesquisar** filtra todas as pastas e arquivos de uma vez, mostrando os resultados como caminhos completos; clicar no cabeçalho de uma coluna ordena por ela, e clicar de novo inverte a ordem. **Desmarcar tudo** limpa a seleção sem mexer em **Usar banco de dados completo**.

O Audfprint só pode ser apontado para uma pasta, então salvar uma seleção **move** fisicamente os arquivos `.pklz` escolhidos para uma pasta `___TEMP` dentro do seu banco de dados, que espelha a estrutura dele, e a varredura busca nessa pasta. Desmarcá-los os move de volta, e **Usar banco de dados completo** devolve tudo e apaga o `___TEMP` por completo, de modo que cada arquivo está sempre no seu lugar original ou em `___TEMP`, nunca em outro lugar. As movimentações acontecem quando você pressiona **Salvar**, atrás de uma janela de progresso, e são reaplicadas no início de cada varredura, para que a pasta de preparação sempre corresponda ao que foi salvo mesmo que arquivos tenham sido adicionados ou movidos manualmente nesse meio-tempo. Nada é sobrescrito: se um arquivo já existe no destino, a movimentação é ignorada e um `[AVISO]` nomeando-o é impresso.

Para mudar quais músicas estão pendentes, abra **Selecionar músicas...** (veja a **aba Geral** acima): ela mostra cada música do seu Diretório de Entrada como uma árvore pesquisável e ordenável, com uma caixa **Rápido** e uma **Longo** em cada linha, além de uma coluna **Tipo de arquivo** (vazia para pastas) e colunas **Arquivos** / **Tamanho** (agregadas através das pastas). Clicar na caixa de uma pasta alterna todas as músicas dentro dela de uma vez. O campo **Pesquisar** filtra músicas e pastas ao mesmo tempo (correspondendo ao caminho relativo); clicar no cabeçalho de uma coluna ordena por ela, e clicar de novo inverte a ordem. Esta janela é onde agora vive o antigo fluxo de trabalho "Marcar todos os arquivos de áudio como processados em", sem precisar depois editar um arquivo de texto manualmente.
...
- **WerZatSonGUI v2.1.0** por some random account, com contribuições de EierkuchenHD, VoidGod, Mystic65 e Numerophobe. Testadores: EierkuchenHD, VoidGod, Shardanik, AuDriūnas, Cluttic, Simon Le Plot, drpostal, gabry4072_, Mystic65. Tradução para o português brasileiro: (W.I.P.).
